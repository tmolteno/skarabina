...
# wheel and must be compiled from source.  Its build system unconditionally
# passes -msse2/-mavx2 which are x86-only flags; disable them globally so the
# compiler does not error out regardless of which pip invocation triggers the
# build (skarabina pulls numcodecs transitively via zarr -> dask-ms).
# These env vars are harmless on x86_64 where pre-built wheels are used.
