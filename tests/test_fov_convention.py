...
def _reference(nu_max_hz, uv_max_m, ell_rad, loss):
    """The Wijnholds (2018) limit, written directly in terms of ℓ."""
    return (
        C_MS
        * (6.0 * loss) ** 0.5
...
def test_field_of_view_is_full_width():
    ...
    assert got == pytest.approx(_reference(1.4e9, 1000.0, math.radians(1.0), 0.01))
...
def test_loss_scaling(loss):
    ...
    assert got == pytest.approx(_reference(1.4e9, 1000.0, math.radians(1.0), loss))
